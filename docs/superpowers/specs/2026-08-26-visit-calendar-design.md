...
4. (2026-09-07 추가) 호스트가 관리 화면에서 **회원을 직접 추가**할 수도 있다 — 아이디·닉네임·초기 비밀번호를 호스트가 정해 본인에게 알려준다. 초대코드 가입과 병행
5. (2026-09-07 변경) **로그인은 이름 + 숫자 4자리 비밀번호**. 별도 아이디·닉네임을 두지 않고 `loginId` = `nickname` = 이름(한글·영문·숫자 2~20자). 동명이인은 뒤에 숫자를 붙인다
6. (2026-09-07 추가) 호스트는 **2명**(선동현·노성소)일 수 있다. 둘 다 관리·승인 권한을 갖고 참석자·랭킹에서 제외된다

### 5.2 로그인

- **아이디 + 비밀번호** (bcrypt, cost 12). 이메일 없음
- NextAuth Credentials + JWT 세션
- 비밀번호 분실 → **호스트가 관리 화면에서 임시 비밀번호 발급**. NAS에서는 호스트가 컨테이너 로그를 볼 수 없어 임시 비번을 관리자 화면에 한 번만 표시한다
...
4. 가입은 **초대코드 필수** 또는 **호스트가 직접 추가** (2026-09-07)
